...
public class JobSeekerController {
    ...
    public ResponseEntity<HttpRes> save(@Valid @RequestBody JobSeekerReq req) {
        ...
        return ResponseEntity.created(URI.create("")).body(HttpRes.builder().timeStamp(LocalDateTime.now().toString()).statusCode(HttpStatus.CREATED.value()).path("myrh/api/v1/jobSeekers").status(HttpStatus.CREATED).message("JobSeeker has been Created").developerMessage("JobSeeker  has been Created").data(Map.of("response", response)).build());
    }
    ...
    public ResponseEntity<HttpRes> auth(@Valid @RequestBody AuthReq auth) {
        ...
        return ResponseEntity.accepted().body(HttpRes.builder().timeStamp(LocalDateTime.now().toString()).statusCode(HttpStatus.ACCEPTED.value()).path("myrh/api/v1/jobSeekers").status(HttpStatus.ACCEPTED).message("JobSeeker has been authenticated").developerMessage("JobSeeker  has been authenticated").data(Map.of("response", response)).build());
    }
    ...
    @GetMapping("/filter")
    public ResponseEntity<Page<JobSeekerRes>> filterAll(@RequestParam Map<String, String> params) {
